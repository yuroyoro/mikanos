...
#### 10.2 はじめてのウィンドウ(osbook_day10b)

ウィンドウを作りますよ。といってもこの段階では単に描画するだけで、動かしたり閉じたりはできない

- `window.hpp` `window.cpp` : `DrawWindow` でウィンドウの描画処理を追加
- `main.cpp` : ウィンドウを一枚レイヤーに割り当てて描画する

#### 10.3 高速カウンタ(osbook_day10c)

ウィンドウにメイン関数のループカウントを表示させる.
...
#### 10.6 ウィンドウのドラッグ移動(osbook_day10f)

ウィンドウをドラッグで移動できるようにするよ。

まず、usbマウスドライバからボタンの状態を受け取れるようにする。
mainでusbマウスドライバに設定しているイベントハンドラ( `MouseObserver` ) の引数にボタンの状態を追加して受け取る。
`MouseObserver` 内では、 ボタンを押された座標にあるウィンドウを取得し、ドラッグが継続しているなら `MoveRelative` で押されているウィンドウを移動させる。
ボタンが離されたら、ドラッグ対象のウィンドウをクリアする


#### 10.7 ウィンドウだけドラッグ移動(osbook_day10g)

コンソールのウィンドウがドラッグできてしまうので、ドラッグできないレイヤーを設定できるようにする。
